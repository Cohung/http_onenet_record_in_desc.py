...
param = {YC.MOBILE:'13026632336',YC.TEXT:'【易联物联科技】机器编号：083780741,货道2缺货，请及时补充'}
...
while True:
		...
		print u"这是第"+str(request_num).encode('utf-8')+u"次查询！"
		#知乎：print (unicode("请输入销售额", encoding="utf-8")) 将utf-8编码转换为unicode就可以输出中文了。
		__file__ = open(r'log.txt', 'a')
		print >>__file__, "这是第"+str(request_num)+"次查询！"
		__file__.close()
		...
		if sms_num > 0 :
			...
			for sms in range(0,sms_num):
					...
					param[YC.TEXT] = '【易联物联科技】机器编号：' + i_str[1].encode('utf-8') + ',货道' + i_str[2].encode('utf-8') + '缺货，请及时补充'				
					#print param[YC.TEXT]
		...
		print u"这次查到了"+str(num).encode('utf-8')+u"条记录,下面开始逐个删除。"
		__file__ = open(r'log.txt', 'a')
		print >>__file__, "这次查到了"+str(num)+"条记录,下面开始逐个删除。"
		__file__.close()
		#print(num)
		...
		for x in range(0,num):
				...
				try:
						...
						print  'Send Failed'
						sys.exit()
				print u"删除第"+str(x+1).encode('utf-8')+u"条记录!"
				time.sleep(0.05) #休息50ms
		print u"这次查到了"+str(sms_num).encode('utf-8')+u"条短信,下面开始逐个删除。"
		__file__ = open(r'log.txt', 'a')
		print >>__file__, "这次查到了"+str(sms_num)+"条短信,下面开始逐个删除。"
		__file__.close()
		for x in range(0,sms_num):
				...
				try:
						...
						print  'Send Failed'
						sys.exit()
				print u"删除第"+str(x+1).encode('utf-8')+u"条短信!"
				time.sleep(0.05) #休息50ms
